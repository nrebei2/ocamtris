(* mutable state of [game.timer]. [timers.update] is the interval in seconds
   from the last frame to the current one. [timers.drop_timer] is the
   interval in seconds from the last call of moving the pieces down.
   [timers.bot_timer] is the interval in seconds from the bot's last
   update *)
type timers = {
...
(* mutable state of the active game. [game.over] is [true] if the game is
   over, [false] otherwise. [game.players] is the gams's players.
